...
class HuggingFace(LanguageModel):
    ...
    def extend_eos_tokens(self):
        # Encode the closing brace safely
        tokens = self.tokenizer.encode("}")
        
        if not tokens:
            # fallback: use some default token id (for example 1) if tokenizer fails
            closing_brace_token = 1
            print("Warning: tokenizer returned empty for '}', using fallback token id 1")
        else:
            # Use second token if exists, otherwise first
            closing_brace_token = tokens[1] if len(tokens) > 1 else tokens[0]
        ...
        print("extend_eos_tokens called, tokens:", tokens, "=> closing_brace_token:", closing_brace_token)
