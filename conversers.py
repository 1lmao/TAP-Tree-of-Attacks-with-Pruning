...
def load_attack_and_target_models(args):
    # Pin attacker to GPU 0 and target to GPU 1
    attack_device = 0
    target_device = 1
...
class AttackLLM():
    ...
    def __init__(self, 
        ...
        # load_indiv_model now supports device_index, load_in_8bit, offload_folder
...
# device_index: if provided, pins the model to that GPU index
# loqd_in_8bit: if True, loads the model in 8-bit quantization using bitsandbytes
def load_indiv_model(model_name, device_index=None, load_in_8bit=False, offload_folder=None):
    ...
    if model_name in ["gpt-3.5-turbo", "gpt-4", 'gpt-4-1106-preview']:
        ...
        # Build a device_map that pins the entire model to a single GPU if device_index provided
