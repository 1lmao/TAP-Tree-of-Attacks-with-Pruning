...
class WandBLogger:
    ...
    def log(self, iteration: int, attack_list: list, response_list: list, judge_scores: list, on_topic_scores: list = None, conv_ids: list = None, parent_conv_ids: list = None):
        ...
        self.logger.log({
            ...
            "data": wandb.Table(data = self.table.assign(score=pd.to_numeric(self.table.get("score", pd.Series([-1]*len(self.table))), errors="coerce")))})
    ...
    def print_final_summary_stats(self):
        ...
        if "score" in self.table.columns:
            self.table["score"] = pd.to_numeric(self.table["score"], errors="coerce")
